...
class DailyEmployeeReport(models.Model):
    ...
    is_late = fields.Boolean(
        string="Submitted Late", compute="_compute_is_late", store=True
    )
        ...
        for rec in self:
            if rec.submitted_at and rec.report_date:
                deadline = datetime.combine(
                    rec.report_date + timedelta(days=1),
        ...
        for rec in self:
            if rec.report_date == now.date() and now.hour < 12:
                raise ValidationError(
                    _("Reports can only be submitted after 12:00 on the same day.")
                )
    ...
    _sql_constraints = [
        (
            ...
            "You have already submitted a report for this day.",
        )
    ]
    ...
    def create(self, vals):
        ...
        if not (is_admin or is_hr or is_supervisor):
            ...
            if now < limit_start or now > limit_end:
                raise ValidationError(
                    "You can only submit your report between 12:00 PM and 09:00 AM next day."
                )
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if not (is_hr or is_supervisor or is_admin):
                if (
                    "supervisor_rating" in vals
                    or "supervisor_comment" in vals
                    or "hr_rating" in vals
                    or "hr_comment" in vals
                ):
                    raise ValidationError(
                        "You are not allowed to modify supervisor or HR ratings."
                    )
            ...
            if is_supervisor and not is_hr:
                if "supervisor_rating" in vals or "supervisor_comment" in vals:
                    if record.employee_id.parent_id.user_id != user:
                        raise ValidationError(
                            "Supervisors can only rate employees from their own department."
                        )
...
class HREmployee(models.Model):
    ...
    daily_report_required = fields.Boolean(
        string="Daily Report Required", default=False
    )
    daily_report_notify_time = fields.Float(
        string="Notification Time (24h format)",
        default=8.5,
